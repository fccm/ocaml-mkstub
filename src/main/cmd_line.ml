...
let tpl_read_file = Utils.read_file_join ;;

let tpl_e a fn =
  let template_e = tpl_read_file fn in
  let template_e_vars = Stmpl.list_vars template_e in
  { a with template_e ; template_e_vars }

let tpl_f a fn =
  let template_f = tpl_read_file fn in
  let template_f_vars = Stmpl.list_vars template_f in
  { a with template_f ; template_f_vars }

let tpl_s a fn =
  let template_s = tpl_read_file fn in
  let template_s_vars = Stmpl.list_vars template_s in
  { a with template_s ; template_s_vars }

let tpl_u a fn =
  let template_u = tpl_read_file fn in
  let template_u_vars = Stmpl.list_vars template_u in
  { a with template_u ; template_u_vars }

let tpl_o a fn =
  let template_o = tpl_read_file fn in
  let template_o_vars = Stmpl.list_vars template_o in
  { a with template_o ; template_o_vars }

let tpl_w a fn =
  let template_w = tpl_read_file fn in
  let template_w_vars = Stmpl.list_vars template_w in
  { a with template_w ; template_w_vars }
...
let long_options = [
  ...
  ("-tp-u", "--union-template");
  ("-tp-o", "--object-template");
  ("-tp-w", "--wrapping-template");
...
let parse_argv argv =
  ...
  and instr a ins = { a with instructions = ins :: a.instructions }
  in
  ...
  | "-tp-o" :: tp :: args -> aux (tpl_o a tp) args
  | "-tp-w" :: tp :: args -> aux (tpl_w a tp) args
