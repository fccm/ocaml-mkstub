...
let list_map a f lst =
  if a.failsafe
  then Utils.lst_map_failsafe f lst
  else Utils.lst_map f lst
...
let read_arraytype a attrs childs =
  ...
  in
  {
    ...
    gx_ar_min    = assoc "min";
    gx_ar_max    = assoc "max";
    gx_ar_type   = assoc "type";
    gx_ar_size   = assoc "size";
    gx_ar_align  = assoc "align";
  }
(*
...
let read_function a attrs childs =
  ...
  in
  {
    ...
    gx_f_args = Utils.lst_map (read_argument a) childs;
  }
...
let read_method a attrs childs =
  ...
  {
    gx_m_id      = assoc "id";
    gx_m_name    = assoc "name";
    gx_m_returns = assoc "returns";
    gx_m_context = assoc "context";
    gx_m_access  = assoc "access";
    gx_m_inline  = assoc "inline";
    gx_m_args = Utils.lst_map (read_argument a) childs;
  }
(*
  ...
  file="f3" line="536" endline="536" inline="1">
    <Argument name="__newval" type="_29" location="f3:536" file="f3" line="536"/>
  </Method>
*)
...
let read_operatormethod a attrs childs =
  ...
  in
  {
    ...
    *)
    gx_o_args = Utils.lst_map (read_argument a) childs;
  }
(*
...
let read_constructor a attrs childs =
  ...
  in
  {
    ...
    gx_c_args = Utils.lst_map (read_argument a) childs;
    (* TODO?
      artificial
      throw
      context
