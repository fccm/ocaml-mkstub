...
let map_arraytype ar =
  ...
  (id, H.ArrayType (arraytype_of_gx ar))
...
let arg_map_types ts arg =
  (*
  { arg with arg_t = List.assoc arg.arg_t ts }
  *)
  { arg with
    arg_t =
      Utils.assoc_default_warn
        arg.arg_t ts
        (H.TODO "TODO (map)")
        (Printf.sprintf "Arg: %s not found" arg.arg_t)
  }
...
let map_struct a members st =
  ...
  (id, H.Struct (struct_of_gx a members st))
...
let map_union a members u =
  let id = u.gx_u_id in
  (id, H.Union (union_of_gx a members u))
...
let map_pointer ptr =
  ...
  (id, H.Ptr (pointer_of_gx ptr))

let qual q =
  assert (List.length q = 1);
  if List.mem "const" q then H.Const else
  if List.mem "volatile" q then H.Volatile else
  if List.mem "restrict" q then H.Restrict else
...
let collect_members gccxml =
  List.fold_left (fun members -> function
    | G.Field fld ->
        ...
        (id, H.Field(map_field fld)) :: members

    | G.Constructor c ->
        let id = c.gx_c_id in
        (id, H.Constructor(map_constructor c)) :: members

    | G.Destructor d ->
        let id = d.gx_d_id in
        (id, H.Destructor(map_destructor d)) :: members

    | G.Method m ->
        let id = m.gx_m_id in
        (id, H.Method(map_method m)) :: members

    | G.OperatorMethod o ->
        let id = o.gx_o_id in
        (id, H.OperatorMethod(map_operatormethod o)) :: members
