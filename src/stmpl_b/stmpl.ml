...
let seq_rep rep_chars s =
  ...
  let rec aux i j m acc =
    ...
    (* XXX ignore other repeat tags inside one *)
...
let rec make_repeat vc rc (ls : param list) acc = function [] -> acc
  | `REP_S rep_s ::
    `REP_J rep_j ::
    `REP_M rep_m ::
    `STR s ::
    ...
    when rep_s = rep_e ->
      ...
      *)
      ...
      let e =
        ...
        let rrls = List.map (fun rep_arg ->
          str ~var_chars:vc ~rep_chars:rc s rep_arg) rep_args in
        String.concat rep_j rrls
      in
      make_repeat vc rc ls (e::acc) tl
  | `STR s :: tl -> make_repeat vc rc ls (s::acc) tl
(*
  | `STR s :: tl -> make_repeat vc rc ls (debug_rep_tag "STR" s::acc) tl
*)
  | `REP_S s :: tl -> make_repeat vc rc ls (debug_rep_tag "REP_S" s ::acc) tl
  | `REP_E s :: tl -> make_repeat vc rc ls (debug_rep_tag "REP_E" s ::acc) tl
  | `REP_J s :: tl -> make_repeat vc rc ls (debug_rep_tag "REP_J" s ::acc) tl
  | `REP_M s :: tl -> make_repeat vc rc ls (debug_rep_tag "REP_M" s ::acc) tl
...
and str
  ...
  *)
  ...
  let rr = make_repeat var_chars rep_chars ls [] (seq_rep rep_chars r) in
  let rrr =
